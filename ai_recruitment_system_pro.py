...
from ai_recruitment_agent_team import (
    init_session_state, create_resume_analyzer, create_email_agent, 
    create_scheduler_agent, extract_text_from_pdf, analyze_resume,
    send_selection_email, send_rejection_email, schedule_interview,
    ROLE_REQUIREMENTS, sanitize_ascii
)
                ...
                try:
                    ...
                    if is_selected:
                        ...
                        if st.session_state.get('show_email_preview') and st.session_state.get('email_type') == 'selection':
                            ...
                            # Generate email preview using the actual email agent
                            try:
                                # Create a temporary email agent to generate the email content
                                temp_email_agent = create_email_agent()
                                
                                # Generate the actual email content that would be sent
                                email_prompt = f"""
                                Send an email to {candidate_email} about selection for the {role} position.
                                Congratulate them and mention next steps.
                                Include company name: {st.session_state.company_name}.
                                """
                                
                                # Get the actual email content
                                email_content = temp_email_agent.run(email_prompt)
                        ...
                        if st.session_state.get('show_email_preview') and st.session_state.get('email_type') == 'rejection':
                            ...
                            # Generate rejection email preview using the actual email agent
                            try:
                                # Create a temporary email agent to generate the email content
                                temp_email_agent = create_email_agent()
                                ...
                                # Generate the actual email content that would be sent
                                email_prompt = f"""
                                send an email to {candidate_email} regarding the {role} application.
                                Use all lowercase, be empathetic and human.
                                Mention feedback: {feedback}
